<template>
  <Popover class="relative">
    <PopoverButton class="inline-flex items-center gap-x-1 p-2">
      Educational
      <ChevronDownIcon class="h-5 w-5" aria-hidden="true" />
    </PopoverButton>

    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
      <PopoverPanel class="absolute left-1/2 z-10 mt-4 flex w-screen max-w-max -translate-x-1/2 px-4">
...
const resources = [
  { name: 'Knowledge Hub', description: 'Find solutions for your questions', href: '#', icon: LifebuoyIcon },
  { name: 'Blog', description: 'Discover helpful insights and information', href: '#', icon: BookmarkSquareIcon },
]
const recentPosts = [
