          ...
          <div class="w-full relative" ref="tabsContainer">
            <div v-for="(tab, index) in tabs" :key="index" class="absolute w-full">
                <div v-if="activeTab === index">
                  ...
                  <img :src="tab.tabImage" alt="" class="absolute w-2/4 h-auto -z-10 right-0 -top-10" aria-hidden="true" v-motion :initial="{ opacity: 0}" :enter="{ opacity: 1, scale: 1, transition: {duration: 600} }" :leave="{ opacity: 0}" />
                </div>
            </div>
          </div>
        </div>
...
const tabs = [
  ...
  },
  {
    ...
    heading: 'Delegate Assets',
    content: 'Delegate tab content',
    tabImage: LeaseImage,
    buttonLink: '#',
  },
  {
    tabName: 'Govern',
    tabIcon: GovernIcon,
    heading: 'Govern Assets',
    content: 'Govern tab content',
    tabImage: LeaseImage,
    buttonLink: '#',
  },
]
